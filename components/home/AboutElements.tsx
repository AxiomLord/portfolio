...
export default function AboutElements() {
  ...
  return (
    <main
      ...
      {...componentAttrs(
        "AboutElements",
        "Agent systems about page with a filterable skills table and workflow reveal.",
      )}
    >
      <header className="flex flex-col gap-3.75">
        ...
        >
          I&apos;m a senior agentic AI engineer working across LLM applications,
          multi-agent systems, React, Node.js, Python, and AWS.
        </p>
      </header>
      ...
      <div className="flex flex-col gap-6.5 min-[901px]:gap-4.5 min-[1201px]:grid min-[1201px]:grid-cols-[minmax(0,1fr)_292px] min-[1201px]:items-start min-[1201px]:gap-10.5">
        <div className="order-2 flex min-w-0 flex-col gap-6.5 min-[901px]:order-1 min-[901px]:gap-8.5">
          <section
            aria-label="The long way to agentic AI"
          ...
          >
            <div className="flex flex-col gap-2">
              ...
              >
                Came to agentic AI through the long way:{" "}
                <button
                  type="button"
                  aria-pressed={agentPinned}
                  onPointerEnter={(event) => {
                    if (event.pointerType !== "touch") {
                      setAgentHovered(true);
                    }
                  }}
                  onPointerLeave={() => setAgentHovered(false)}
                  onClick={() => setAgentPinned((current) => !current)}
                  className={cn(
                    ...
                    agentPinned && "border-accent text-accent",
                  )}
                >
                  eight years of product engineering
                </button>{" "}
                across healthcare, analytics, fintech, and full-stack product
                work, then followed the systems into AI.
              </p>
